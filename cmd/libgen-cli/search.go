...
var searchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for {
			...
			if len(books) == 0 {
				fmt.Printf("\nNo results found for page %d from: %s.\n", currentPage, searchMirror.String())
				if currentPage > 1 {
					fmt.Println("Returning to previous page...")
					currentPage--
					continue
				}
				os.Exit(1)
			...
			for _, b := range books {
				selectChoice := fmt.Sprintf("%8s ", color.New(color.FgHiBlue).Sprint(b.ID))
				if len(b.Title) > 36 {
				...
				if b.Author != "" {
					if len(b.Author) > 20 {
						selectChoice += fmt.Sprintf("%s ", color.New(color.FgYellow).Sprint(b.Author[:17]+"..."))
					} else {
						selectChoice += fmt.Sprintf("%s ", color.New(color.FgYellow).Sprint(b.Author))
					}
				} else {
					selectChoice += fmt.Sprintf("%s ", color.New(color.FgYellow).Sprint("N/A"))
				}
				selectChoice += fmt.Sprintf("| %-4s ", color.New(color.FgRed).Sprint(b.Extension))
				size, err := strconv.Atoi(b.Filesize)
				if err != nil {
					size = 0
				}
				selectChoice += fmt.Sprintf("| %v", color.New(color.FgGreen).Sprint(humanize.Bytes(uint64(size))))
				bookSelection = append(bookSelection, selectChoice)
			}

			// Add pagination controls to selection options
			nextChoice := color.New(color.FgCyan).Sprintf("[➡  Next Page (Page %d)]", currentPage+1)
			bookSelection = append(bookSelection, nextChoice)
			var prevChoice string
			if currentPage > 1 {
				prevChoice = color.New(color.FgCyan).Sprintf("[⬅  Previous Page (Page %d)]", currentPage-1)
				bookSelection = append(bookSelection, prevChoice)
			}
			...
			prompt := promptui.Select{
				Label:     fmt.Sprintf("Select Book (Page %d)", currentPage),
				Items:     bookSelection,
				Templates: promptTemplate,
				Size:      len(bookSelection),
				IsVimMode: false,
			...
			choiceIdx, result, err := prompt.Run()
			if err != nil {
				fmt.Println(err)
				os.Exit(1)
			}

			// Check if user selected Next Page
			if strings.Contains(result, "Next Page") {
				currentPage++
				continue
			}
			// Check if user selected Previous Page
			if strings.Contains(result, "Previous Page") {
				if currentPage > 1 {
					currentPage--
				}
				continue
			...
			if choiceIdx >= 0 && choiceIdx < len(books) {
				selectedBook = *books[choiceIdx]
			} else {
				for i, b := range bookSelection {
					if b == result && i < len(books) {
						selectedBook = *books[i]
						break
					}
				}
			}
			if selectedBook.ID == "" && len(books) > 0 {
...
func init() {
	...
	searchCmd.Flags().IntP("page", "P", 1, "page number of search results to display.")
	searchCmd.Flags().StringP("type", "t", "", "filters search query results by book type / file extension (e.g. epub, pdf, mobi).")
	searchCmd.Flags().String("book-type", "", "alias for --type (e.g. epub, pdf, mobi).")
	searchCmd.Flags().Int("max-pages", 5, "maximum pages to search when filtering results.")
	searchCmd.Flags().Bool("auto-page", true, "automatically search across subsequent pages to find matching results.")
	...
	searchCmd.Flags().StringP("mirror", "m", "", "custom LibGen mirror URL")
}
