...
var downloadCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		re := regexp.MustCompile(libgen.ValidMD5)
		...
		if err != nil {
			// If error and no custom mirror was forced, try candidate mirrors once
			if customMirror == "" {
				for _, candidate := range libgen.SearchMirrors {
					if candidate.Host != searchMirror.Host {
						bookDetails, err = libgen.GetDetails(&libgen.GetDetailsOptions{
							Hashes:       args,
							SearchMirror: candidate,
							Print:        true,
						})
			...
			if err != nil {
				fmt.Printf("error retrieving results from LibGen API: %v\n", err)
				os.Exit(1)
			}
		}
		if len(bookDetails) == 0 {
			fmt.Printf("No books found matching provided hash(es)\n")
			os.Exit(1)
		}
...
func init() {
	...
	downloadCmd.Flags().StringP("mirror", "m", "", "custom LibGen mirror URL")
}
